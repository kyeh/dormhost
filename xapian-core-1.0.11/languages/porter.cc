/* This file was generated automatically by the Snowball to ISO C++ compiler */

#include <limits.h>
#include "porter.h"

static const symbol s_0_0[1] = { 's' };
static const symbol s_0_1[3] = { 'i', 'e', 's' };
static const symbol s_0_2[4] = { 's', 's', 'e', 's' };
static const symbol s_0_3[2] = { 's', 's' };

static const struct among a_0[4] =
{
/*  0 */ { 1, s_0_0, -1, 3},
/*  1 */ { 3, s_0_1, 0, 2},
/*  2 */ { 4, s_0_2, 0, 1},
/*  3 */ { 2, s_0_3, 0, -1}
};

static const symbol s_1_1[2] = { 'b', 'b' };
static const symbol s_1_2[2] = { 'd', 'd' };
static const symbol s_1_3[2] = { 'f', 'f' };
static const symbol s_1_4[2] = { 'g', 'g' };
static const symbol s_1_5[2] = { 'b', 'l' };
static const symbol s_1_6[2] = { 'm', 'm' };
static const symbol s_1_7[2] = { 'n', 'n' };
static const symbol s_1_8[2] = { 'p', 'p' };
static const symbol s_1_9[2] = { 'r', 'r' };
static const symbol s_1_10[2] = { 'a', 't' };
static const symbol s_1_11[2] = { 't', 't' };
static const symbol s_1_12[2] = { 'i', 'z' };

static const struct among a_1[13] =
{
/*  0 */ { 0, 0, -1, 3},
/*  1 */ { 2, s_1_1, 0, 2},
/*  2 */ { 2, s_1_2, 0, 2},
/*  3 */ { 2, s_1_3, 0, 2},
/*  4 */ { 2, s_1_4, 0, 2},
/*  5 */ { 2, s_1_5, 0, 1},
/*  6 */ { 2, s_1_6, 0, 2},
/*  7 */ { 2, s_1_7, 0, 2},
/*  8 */ { 2, s_1_8, 0, 2},
/*  9 */ { 2, s_1_9, 0, 2},
/* 10 */ { 2, s_1_10, 0, 1},
/* 11 */ { 2, s_1_11, 0, 2},
/* 12 */ { 2, s_1_12, 0, 1}
};

static const symbol s_2_0[2] = { 'e', 'd' };
static const symbol s_2_1[3] = { 'e', 'e', 'd' };
static const symbol s_2_2[3] = { 'i', 'n', 'g' };

static const struct among a_2[3] =
{
/*  0 */ { 2, s_2_0, -1, 2},
/*  1 */ { 3, s_2_1, 0, 1},
/*  2 */ { 3, s_2_2, -1, 2}
};

static const symbol s_3_0[4] = { 'a', 'n', 'c', 'i' };
static const symbol s_3_1[4] = { 'e', 'n', 'c', 'i' };
static const symbol s_3_2[4] = { 'a', 'b', 'l', 'i' };
static const symbol s_3_3[3] = { 'e', 'l', 'i' };
static const symbol s_3_4[4] = { 'a', 'l', 'l', 'i' };
static const symbol s_3_5[5] = { 'o', 'u', 's', 'l', 'i' };
static const symbol s_3_6[5] = { 'e', 'n', 't', 'l', 'i' };
static const symbol s_3_7[5] = { 'a', 'l', 'i', 't', 'i' };
static const symbol s_3_8[6] = { 'b', 'i', 'l', 'i', 't', 'i' };
static const symbol s_3_9[5] = { 'i', 'v', 'i', 't', 'i' };
static const symbol s_3_10[6] = { 't', 'i', 'o', 'n', 'a', 'l' };
static const symbol s_3_11[7] = { 'a', 't', 'i', 'o', 'n', 'a', 'l' };
static const symbol s_3_12[5] = { 'a', 'l', 'i', 's', 'm' };
static const symbol s_3_13[5] = { 'a', 't', 'i', 'o', 'n' };
static const symbol s_3_14[7] = { 'i', 'z', 'a', 't', 'i', 'o', 'n' };
static const symbol s_3_15[4] = { 'i', 'z', 'e', 'r' };
static const symbol s_3_16[4] = { 'a', 't', 'o', 'r' };
static const symbol s_3_17[7] = { 'i', 'v', 'e', 'n', 'e', 's', 's' };
static const symbol s_3_18[7] = { 'f', 'u', 'l', 'n', 'e', 's', 's' };
static const symbol s_3_19[7] = { 'o', 'u', 's', 'n', 'e', 's', 's' };

static const struct among a_3[20] =
{
/*  0 */ { 4, s_3_0, -1, 3},
/*  1 */ { 4, s_3_1, -1, 2},
/*  2 */ { 4, s_3_2, -1, 4},
/*  3 */ { 3, s_3_3, -1, 6},
/*  4 */ { 4, s_3_4, -1, 9},
/*  5 */ { 5, s_3_5, -1, 12},
/*  6 */ { 5, s_3_6, -1, 5},
/*  7 */ { 5, s_3_7, -1, 10},
/*  8 */ { 6, s_3_8, -1, 14},
/*  9 */ { 5, s_3_9, -1, 13},
/* 10 */ { 6, s_3_10, -1, 1},
/* 11 */ { 7, s_3_11, 10, 8},
/* 12 */ { 5, s_3_12, -1, 10},
/* 13 */ { 5, s_3_13, -1, 8},
/* 14 */ { 7, s_3_14, 13, 7},
/* 15 */ { 4, s_3_15, -1, 7},
/* 16 */ { 4, s_3_16, -1, 8},
/* 17 */ { 7, s_3_17, -1, 13},
/* 18 */ { 7, s_3_18, -1, 11},
/* 19 */ { 7, s_3_19, -1, 12}
};

static const symbol s_4_0[5] = { 'i', 'c', 'a', 't', 'e' };
static const symbol s_4_1[5] = { 'a', 't', 'i', 'v', 'e' };
static const symbol s_4_2[5] = { 'a', 'l', 'i', 'z', 'e' };
static const symbol s_4_3[5] = { 'i', 'c', 'i', 't', 'i' };
static const symbol s_4_4[4] = { 'i', 'c', 'a', 'l' };
static const symbol s_4_5[3] = { 'f', 'u', 'l' };
static const symbol s_4_6[4] = { 'n', 'e', 's', 's' };

static const struct among a_4[7] =
{
/*  0 */ { 5, s_4_0, -1, 2},
/*  1 */ { 5, s_4_1, -1, 3},
/*  2 */ { 5, s_4_2, -1, 1},
/*  3 */ { 5, s_4_3, -1, 2},
/*  4 */ { 4, s_4_4, -1, 2},
/*  5 */ { 3, s_4_5, -1, 3},
/*  6 */ { 4, s_4_6, -1, 3}
};

static const symbol s_5_0[2] = { 'i', 'c' };
static const symbol s_5_1[4] = { 'a', 'n', 'c', 'e' };
static const symbol s_5_2[4] = { 'e', 'n', 'c', 'e' };
static const symbol s_5_3[4] = { 'a', 'b', 'l', 'e' };
static const symbol s_5_4[4] = { 'i', 'b', 'l', 'e' };
static const symbol s_5_5[3] = { 'a', 't', 'e' };
static const symbol s_5_6[3] = { 'i', 'v', 'e' };
static const symbol s_5_7[3] = { 'i', 'z', 'e' };
static const symbol s_5_8[3] = { 'i', 't', 'i' };
static const symbol s_5_9[2] = { 'a', 'l' };
static const symbol s_5_10[3] = { 'i', 's', 'm' };
static const symbol s_5_11[3] = { 'i', 'o', 'n' };
static const symbol s_5_12[2] = { 'e', 'r' };
static const symbol s_5_13[3] = { 'o', 'u', 's' };
static const symbol s_5_14[3] = { 'a', 'n', 't' };
static const symbol s_5_15[3] = { 'e', 'n', 't' };
static const symbol s_5_16[4] = { 'm', 'e', 'n', 't' };
static const symbol s_5_17[5] = { 'e', 'm', 'e', 'n', 't' };
static const symbol s_5_18[2] = { 'o', 'u' };

static const struct among a_5[19] =
{
/*  0 */ { 2, s_5_0, -1, 1},
/*  1 */ { 4, s_5_1, -1, 1},
/*  2 */ { 4, s_5_2, -1, 1},
/*  3 */ { 4, s_5_3, -1, 1},
/*  4 */ { 4, s_5_4, -1, 1},
/*  5 */ { 3, s_5_5, -1, 1},
/*  6 */ { 3, s_5_6, -1, 1},
/*  7 */ { 3, s_5_7, -1, 1},
/*  8 */ { 3, s_5_8, -1, 1},
/*  9 */ { 2, s_5_9, -1, 1},
/* 10 */ { 3, s_5_10, -1, 1},
/* 11 */ { 3, s_5_11, -1, 2},
/* 12 */ { 2, s_5_12, -1, 1},
/* 13 */ { 3, s_5_13, -1, 1},
/* 14 */ { 3, s_5_14, -1, 1},
/* 15 */ { 3, s_5_15, -1, 1},
/* 16 */ { 4, s_5_16, 15, 1},
/* 17 */ { 5, s_5_17, 16, 1},
/* 18 */ { 2, s_5_18, -1, 1}
};

static const unsigned char g_v[] = { 17, 65, 16, 1 };

static const unsigned char g_v_WXY[] = { 1, 17, 65, 208, 1 };

static const symbol s_0[] = { 's', 's' };
static const symbol s_1[] = { 'i' };
static const symbol s_2[] = { 'e', 'e' };
static const symbol s_3[] = { 'e' };
static const symbol s_4[] = { 'e' };
static const symbol s_5[] = { 'i' };
static const symbol s_6[] = { 't', 'i', 'o', 'n' };
static const symbol s_7[] = { 'e', 'n', 'c', 'e' };
static const symbol s_8[] = { 'a', 'n', 'c', 'e' };
static const symbol s_9[] = { 'a', 'b', 'l', 'e' };
static const symbol s_10[] = { 'e', 'n', 't' };
static const symbol s_11[] = { 'e' };
static const symbol s_12[] = { 'i', 'z', 'e' };
static const symbol s_13[] = { 'a', 't', 'e' };
static const symbol s_14[] = { 'a', 'l' };
static const symbol s_15[] = { 'a', 'l' };
static const symbol s_16[] = { 'f', 'u', 'l' };
static const symbol s_17[] = { 'o', 'u', 's' };
static const symbol s_18[] = { 'i', 'v', 'e' };
static const symbol s_19[] = { 'b', 'l', 'e' };
static const symbol s_20[] = { 'a', 'l' };
static const symbol s_21[] = { 'i', 'c' };
static const symbol s_22[] = { 'Y' };
static const symbol s_23[] = { 'Y' };
static const symbol s_24[] = { 'y' };

int Xapian::InternalStemPorter::r_shortv() { /* backwardmode */
    if (out_grouping_b_U(g_v_WXY, 89, 121, 0)) return 0; /* non v_WXY, line 19 */
    if (in_grouping_b_U(g_v, 97, 121, 0)) return 0; /* grouping v, line 19 */
    if (out_grouping_b_U(g_v, 97, 121, 0)) return 0; /* non v, line 19 */
    return 1;
}

int Xapian::InternalStemPorter::r_R1() { /* backwardmode */
    if (!(I_p1 <= c)) return 0; /* p1 <= <integer expression>, line 21 */
    return 1;
}

int Xapian::InternalStemPorter::r_R2() { /* backwardmode */
    if (!(I_p2 <= c)) return 0; /* p2 <= <integer expression>, line 22 */
    return 1;
}

int Xapian::InternalStemPorter::r_Step_1a() { /* backwardmode */
    int among_var;
    ket = c; /* [, line 25 */
    if (c <= lb || p[c - 1] != 115) return 0; /* substring, line 25 */
    among_var = find_among_b(a_0, 4, 0, 0);
    if (!(among_var)) return 0;
    bra = c; /* ], line 25 */
    switch(among_var) { /* among, line 25 */
        case 0: return 0;
        case 1:
            {   int ret = slice_from_s(2, s_0); /* <-, line 26 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   int ret = slice_from_s(1, s_1); /* <-, line 27 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            if (slice_del() == -1) return -1; /* delete, line 29 */
            break;
    }
    return 1;
}

int Xapian::InternalStemPorter::r_Step_1b() { /* backwardmode */
    int among_var;
    ket = c; /* [, line 34 */
    if (c - 1 <= lb || (p[c - 1] != 100 && p[c - 1] != 103)) return 0; /* substring, line 34 */
    among_var = find_among_b(a_2, 3, 0, 0);
    if (!(among_var)) return 0;
    bra = c; /* ], line 34 */
    switch(among_var) { /* among, line 34 */
        case 0: return 0;
        case 1:
            {   int ret = r_R1(); /* call R1, line 35 */
                if (ret <= 0) return ret;
            }
            {   int ret = slice_from_s(2, s_2); /* <-, line 35 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   int m_test1 = l - c; /* test, line 38 */
                {   int ret = out_grouping_b_U(g_v, 97, 121, 1); /* gopast */ /* grouping v, line 38 */
                    if (ret < 0) return 0;
                    c -= ret;
                }
                c = l - m_test1;
            }
            if (slice_del() == -1) return -1; /* delete, line 38 */
            {   int m_test2 = l - c; /* test, line 39 */
                if (c - 1 <= lb || p[c - 1] >> 5 != 3 || !((68514004 >> (p[c - 1] & 0x1f)) & 1)) among_var = 3; else /* substring, line 39 */
                among_var = find_among_b(a_1, 13, 0, 0);
                if (!(among_var)) return 0;
                c = l - m_test2;
            }
            switch(among_var) { /* among, line 39 */
                case 0: return 0;
                case 1:
                    {   int saved_c = c;
                        insert_s(c, c, 1, s_3); /* <+, line 41 */
                        c = saved_c;
                    }
                    break;
                case 2:
                    ket = c; /* [, line 44 */
                    {   int ret = skip_utf8(p, c, lb, 0, -1);
                        if (ret < 0) return 0;
                        c = ret; /* next, line 44 */
                    }
                    bra = c; /* ], line 44 */
                    if (slice_del() == -1) return -1; /* delete, line 44 */
                    break;
                case 3:
                    if (c != I_p1) return 0; /* atmark, line 45 */
                    {   int m_test3 = l - c; /* test, line 45 */
                        {   int ret = r_shortv(); /* call shortv, line 45 */
                            if (ret <= 0) return ret;
                        }
                        c = l - m_test3;
                    }
                    {   int saved_c = c;
                        insert_s(c, c, 1, s_4); /* <+, line 45 */
                        c = saved_c;
                    }
                    break;
            }
            break;
    }
    return 1;
}

int Xapian::InternalStemPorter::r_Step_1c() { /* backwardmode */
    ket = c; /* [, line 52 */
    {   int m1 = l - c; (void)m1; /* or, line 52 */
        if (c <= lb || p[c - 1] != 'y') goto lab1;
        c--;
        goto lab0;
    lab1:
        c = l - m1;
        if (c <= lb || p[c - 1] != 'Y') return 0;
        c--;
    }
lab0:
    bra = c; /* ], line 52 */
    {   int ret = out_grouping_b_U(g_v, 97, 121, 1); /* gopast */ /* grouping v, line 53 */
        if (ret < 0) return 0;
        c -= ret;
    }
    {   int ret = slice_from_s(1, s_5); /* <-, line 54 */
        if (ret < 0) return ret;
    }
    return 1;
}

int Xapian::InternalStemPorter::r_Step_2() { /* backwardmode */
    int among_var;
    ket = c; /* [, line 58 */
    if (c - 2 <= lb || p[c - 1] >> 5 != 3 || !((815616 >> (p[c - 1] & 0x1f)) & 1)) return 0; /* substring, line 58 */
    among_var = find_among_b(a_3, 20, 0, 0);
    if (!(among_var)) return 0;
    bra = c; /* ], line 58 */
    {   int ret = r_R1(); /* call R1, line 58 */
        if (ret <= 0) return ret;
    }
    switch(among_var) { /* among, line 58 */
        case 0: return 0;
        case 1:
            {   int ret = slice_from_s(4, s_6); /* <-, line 59 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   int ret = slice_from_s(4, s_7); /* <-, line 60 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            {   int ret = slice_from_s(4, s_8); /* <-, line 61 */
                if (ret < 0) return ret;
            }
            break;
        case 4:
            {   int ret = slice_from_s(4, s_9); /* <-, line 62 */
                if (ret < 0) return ret;
            }
            break;
        case 5:
            {   int ret = slice_from_s(3, s_10); /* <-, line 63 */
                if (ret < 0) return ret;
            }
            break;
        case 6:
            {   int ret = slice_from_s(1, s_11); /* <-, line 64 */
                if (ret < 0) return ret;
            }
            break;
        case 7:
            {   int ret = slice_from_s(3, s_12); /* <-, line 66 */
                if (ret < 0) return ret;
            }
            break;
        case 8:
            {   int ret = slice_from_s(3, s_13); /* <-, line 68 */
                if (ret < 0) return ret;
            }
            break;
        case 9:
            {   int ret = slice_from_s(2, s_14); /* <-, line 69 */
                if (ret < 0) return ret;
            }
            break;
        case 10:
            {   int ret = slice_from_s(2, s_15); /* <-, line 71 */
                if (ret < 0) return ret;
            }
            break;
        case 11:
            {   int ret = slice_from_s(3, s_16); /* <-, line 72 */
                if (ret < 0) return ret;
            }
            break;
        case 12:
            {   int ret = slice_from_s(3, s_17); /* <-, line 74 */
                if (ret < 0) return ret;
            }
            break;
        case 13:
            {   int ret = slice_from_s(3, s_18); /* <-, line 76 */
                if (ret < 0) return ret;
            }
            break;
        case 14:
            {   int ret = slice_from_s(3, s_19); /* <-, line 77 */
                if (ret < 0) return ret;
            }
            break;
    }
    return 1;
}

int Xapian::InternalStemPorter::r_Step_3() { /* backwardmode */
    int among_var;
    ket = c; /* [, line 82 */
    if (c - 2 <= lb || p[c - 1] >> 5 != 3 || !((528928 >> (p[c - 1] & 0x1f)) & 1)) return 0; /* substring, line 82 */
    among_var = find_among_b(a_4, 7, 0, 0);
    if (!(among_var)) return 0;
    bra = c; /* ], line 82 */
    {   int ret = r_R1(); /* call R1, line 82 */
        if (ret <= 0) return ret;
    }
    switch(among_var) { /* among, line 82 */
        case 0: return 0;
        case 1:
            {   int ret = slice_from_s(2, s_20); /* <-, line 83 */
                if (ret < 0) return ret;
            }
            break;
        case 2:
            {   int ret = slice_from_s(2, s_21); /* <-, line 85 */
                if (ret < 0) return ret;
            }
            break;
        case 3:
            if (slice_del() == -1) return -1; /* delete, line 87 */
            break;
    }
    return 1;
}

int Xapian::InternalStemPorter::r_Step_4() { /* backwardmode */
    int among_var;
    ket = c; /* [, line 92 */
    if (c - 1 <= lb || p[c - 1] >> 5 != 3 || !((3961384 >> (p[c - 1] & 0x1f)) & 1)) return 0; /* substring, line 92 */
    among_var = find_among_b(a_5, 19, 0, 0);
    if (!(among_var)) return 0;
    bra = c; /* ], line 92 */
    {   int ret = r_R2(); /* call R2, line 92 */
        if (ret <= 0) return ret;
    }
    switch(among_var) { /* among, line 92 */
        case 0: return 0;
        case 1:
            if (slice_del() == -1) return -1; /* delete, line 95 */
            break;
        case 2:
            {   int m1 = l - c; (void)m1; /* or, line 96 */
                if (c <= lb || p[c - 1] != 's') goto lab1;
                c--;
                goto lab0;
            lab1:
                c = l - m1;
                if (c <= lb || p[c - 1] != 't') return 0;
                c--;
            }
        lab0:
            if (slice_del() == -1) return -1; /* delete, line 96 */
            break;
    }
    return 1;
}

int Xapian::InternalStemPorter::r_Step_5a() { /* backwardmode */
    ket = c; /* [, line 101 */
    if (c <= lb || p[c - 1] != 'e') return 0;
    c--;
    bra = c; /* ], line 101 */
    {   int m1 = l - c; (void)m1; /* or, line 102 */
        {   int ret = r_R2(); /* call R2, line 102 */
            if (ret == 0) goto lab1;
            if (ret < 0) return ret;
        }
        goto lab0;
    lab1:
        c = l - m1;
        {   int ret = r_R1(); /* call R1, line 102 */
            if (ret <= 0) return ret;
        }
        {   int m2 = l - c; (void)m2; /* not, line 102 */
            {   int ret = r_shortv(); /* call shortv, line 102 */
                if (ret == 0) goto lab2;
                if (ret < 0) return ret;
            }
            return 0;
        lab2:
            c = l - m2;
        }
    }
lab0:
    if (slice_del() == -1) return -1; /* delete, line 103 */
    return 1;
}

int Xapian::InternalStemPorter::r_Step_5b() { /* backwardmode */
    ket = c; /* [, line 107 */
    if (c <= lb || p[c - 1] != 'l') return 0;
    c--;
    bra = c; /* ], line 107 */
    {   int ret = r_R2(); /* call R2, line 108 */
        if (ret <= 0) return ret;
    }
    if (c <= lb || p[c - 1] != 'l') return 0;
    c--;
    if (slice_del() == -1) return -1; /* delete, line 109 */
    return 1;
}

int Xapian::InternalStemPorter::stem() { /* forwardmode */
    B_Y_found = 0; /* unset Y_found, line 115 */
    {   int c1 = c; /* do, line 116 */
        bra = c; /* [, line 116 */
        if (c == l || p[c] != 'y') goto lab0;
        c++;
        ket = c; /* ], line 116 */
        {   int ret = slice_from_s(1, s_22); /* <-, line 116 */
            if (ret < 0) return ret;
        }
        B_Y_found = 1; /* set Y_found, line 116 */
    lab0:
        c = c1;
    }
    {   int c2 = c; /* do, line 117 */
        while(1) { /* repeat, line 117 */
            int c3 = c;
            while(1) { /* goto, line 117 */
                int c4 = c;
                if (in_grouping_U(g_v, 97, 121, 0)) goto lab3; /* grouping v, line 117 */
                bra = c; /* [, line 117 */
                if (c == l || p[c] != 'y') goto lab3;
                c++;
                ket = c; /* ], line 117 */
                c = c4;
                break;
            lab3:
                c = c4;
                {   int ret = skip_utf8(p, c, 0, l, 1);
                    if (ret < 0) goto lab2;
                    c = ret; /* goto, line 117 */
                }
            }
            {   int ret = slice_from_s(1, s_23); /* <-, line 117 */
                if (ret < 0) return ret;
            }
            B_Y_found = 1; /* set Y_found, line 117 */
            continue;
        lab2:
            c = c3;
            break;
        }
        c = c2;
    }
    I_p1 = l; /* p1 = <integer expression>, line 119 */
    I_p2 = l; /* p2 = <integer expression>, line 120 */
    {   int c5 = c; /* do, line 121 */
        {   int ret = out_grouping_U(g_v, 97, 121, 1); /* gopast */ /* grouping v, line 122 */
            if (ret < 0) goto lab4;
            c += ret;
        }
        {   int ret = in_grouping_U(g_v, 97, 121, 1); /* gopast */ /* non v, line 122 */
            if (ret < 0) goto lab4;
            c += ret;
        }
        I_p1 = c; /* setmark p1, line 122 */
        {   int ret = out_grouping_U(g_v, 97, 121, 1); /* gopast */ /* grouping v, line 123 */
            if (ret < 0) goto lab4;
            c += ret;
        }
        {   int ret = in_grouping_U(g_v, 97, 121, 1); /* gopast */ /* non v, line 123 */
            if (ret < 0) goto lab4;
            c += ret;
        }
        I_p2 = c; /* setmark p2, line 123 */
    lab4:
        c = c5;
    }
    lb = c; c = l; /* backwards, line 126 */

    {   int m6 = l - c; (void)m6; /* do, line 127 */
        {   int ret = r_Step_1a(); /* call Step_1a, line 127 */
            if (ret == 0) goto lab5;
            if (ret < 0) return ret;
        }
    lab5:
        c = l - m6;
    }
    {   int m7 = l - c; (void)m7; /* do, line 128 */
        {   int ret = r_Step_1b(); /* call Step_1b, line 128 */
            if (ret == 0) goto lab6;
            if (ret < 0) return ret;
        }
    lab6:
        c = l - m7;
    }
    {   int m8 = l - c; (void)m8; /* do, line 129 */
        {   int ret = r_Step_1c(); /* call Step_1c, line 129 */
            if (ret == 0) goto lab7;
            if (ret < 0) return ret;
        }
    lab7:
        c = l - m8;
    }
    {   int m9 = l - c; (void)m9; /* do, line 130 */
        {   int ret = r_Step_2(); /* call Step_2, line 130 */
            if (ret == 0) goto lab8;
            if (ret < 0) return ret;
        }
    lab8:
        c = l - m9;
    }
    {   int m10 = l - c; (void)m10; /* do, line 131 */
        {   int ret = r_Step_3(); /* call Step_3, line 131 */
            if (ret == 0) goto lab9;
            if (ret < 0) return ret;
        }
    lab9:
        c = l - m10;
    }
    {   int m11 = l - c; (void)m11; /* do, line 132 */
        {   int ret = r_Step_4(); /* call Step_4, line 132 */
            if (ret == 0) goto lab10;
            if (ret < 0) return ret;
        }
    lab10:
        c = l - m11;
    }
    {   int m12 = l - c; (void)m12; /* do, line 133 */
        {   int ret = r_Step_5a(); /* call Step_5a, line 133 */
            if (ret == 0) goto lab11;
            if (ret < 0) return ret;
        }
    lab11:
        c = l - m12;
    }
    {   int m13 = l - c; (void)m13; /* do, line 134 */
        {   int ret = r_Step_5b(); /* call Step_5b, line 134 */
            if (ret == 0) goto lab12;
            if (ret < 0) return ret;
        }
    lab12:
        c = l - m13;
    }
    c = lb;
    {   int c14 = c; /* do, line 137 */
        if (!(B_Y_found)) goto lab13; /* Boolean test Y_found, line 137 */
        while(1) { /* repeat, line 137 */
            int c15 = c;
            while(1) { /* goto, line 137 */
                int c16 = c;
                bra = c; /* [, line 137 */
                if (c == l || p[c] != 'Y') goto lab15;
                c++;
                ket = c; /* ], line 137 */
                c = c16;
                break;
            lab15:
                c = c16;
                {   int ret = skip_utf8(p, c, 0, l, 1);
                    if (ret < 0) goto lab14;
                    c = ret; /* goto, line 137 */
                }
            }
            {   int ret = slice_from_s(1, s_24); /* <-, line 137 */
                if (ret < 0) return ret;
            }
            continue;
        lab14:
            c = c15;
            break;
        }
    lab13:
        c = c14;
    }
    return 1;
}

Xapian::InternalStemPorter::InternalStemPorter()
    : B_Y_found(0), I_p2(0), I_p1(0)
{
}

Xapian::InternalStemPorter::~InternalStemPorter()
{
}

const char *
Xapian::InternalStemPorter::get_description() const
{
    return "porter";
}
